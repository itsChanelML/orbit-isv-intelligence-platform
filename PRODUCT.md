...
- Prepares a workshop outline or technical brief
...
- Produced a tailored deliverable — a Workshop guide, Jupyter Notebook, or Hackathon brief — based on how their team learns
...
The DevRel manager arrives with a complete ISV profile, a generated adoption strategy, and a set of addressed concerns. The conversation starts at a completely different level.
...
- `nvidia/llama-3.3-nemotron-super-49b-v1` — ISV recommendations, concern responses, Orbit chat, workshop and hackathon generation
...
### Output Generation
- 3 context-aware DGX Cloud integration recommendations with exact NIM microservice names, NVIDIA stack components, and partner tool integrations (GCP, Anthropic, Vercel)
- Adoption concern responses with hyperlinked developers.nvidia.com resources
- Primary deliverable generated based on #1 ranked format
- Downloadable Workshop (.md), Hackathon brief (.md / .txt), or Jupyter Notebook (.ipynb)
- Styled Jupyter Notebook browser preview before download

### Persistent ISV Portal
- Orbit chat powered by Nemotron, grounded in the ISV's intake profile and generated outputs
- Tech stack sidebar with GCP Service Usage API auto-detection (polls enabled GCP APIs every 24 hours)
...
- 16 NVIDIA products catalogued with descriptions, use cases, and documentation URLs
- Products include: DGX Cloud, NIM, MONAI, Clara, Clara Holoscan, BioNeMo, NeMo, TensorRT-LLM, Riva, Omniverse, Isaac, CUDA, Triton, Merlin, Metropolis, Aerial
- Catalog is designed to grow — new products are added without touching application code

### Deployment
- Containerized with Docker, deployed on GCP Cloud Run
- The deployment architecture itself demonstrates the NVIDIA + Google Cloud partnership
- Gunicorn WSGI server, 1Gi memory, 2 CPU, 300s timeout for NIM inference calls
...
### Results as Persistent Tabs
- Adoption strategies saved to database per user account
- Each strategy stored with full recommendations, deliverable, concern responses, and metadata
- Browsable history with expand/collapse toggle
...
**It infers** things that were never explicitly stated — their learning style from ranking behavior, which NVIDIA products are most relevant from their use case description, what concerns are likely to block adoption based on their team context.

**It acts** by generating recommendations, producing deliverables, addressing concerns, alerting on new technologies, and delivering reports — without waiting for a human to tell it what to do.
...
*Orbit is a portfolio project built by Chanel Power for the NVIDIA Senior Developer Relations Manager.*
